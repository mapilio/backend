...
4. baseline-free Larastan/PHPStan level 6 over `app`, `bootstrap/app.php`, `database`, and `routes`
...
The complete `database` tree was then measured separately at level 6 and had zero findings. Migrations, seeders, and factories are therefore part of the same permanent gate rather than a second command. The test suite probe found 38 level 6 findings and remains outside the static-analysis path until those test contracts are fixed. A configured-scope level 7 probe found 419 findings: 387 dynamic database-row property findings and 32 other type findings. Level 7 is not enabled and none of those findings is suppressed; query-row boundaries must be modeled deliberately before promotion.
