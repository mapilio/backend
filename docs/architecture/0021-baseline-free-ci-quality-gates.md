...
The initial configured production-scope level 7 probe found 419 findings: 387 dynamic database-row property findings and 32 other type findings. The first bounded promotion replaces `LeaderboardWinnerQuery`'s whole-row challenge lookup with a scalar `is_calculated` projection, preserving missing, false, and true behavior while removing its only dynamic-property finding. The second promotion gives `CalculateSequenceUkmScores` one immutable `UkmDistanceRow` contract across its PostgreSQL and portable query paths. Raw PostgreSQL scalar values are validated before scoring, missing-neighbor `null` remains explicit, and malformed rows fail closed with a bounded domain error. The permanent `analyse:level-7` command covers both promoted workflows in local release checks and GitHub Quality. The remaining production-scope probe has 415 findings: 383 dynamic database-row property findings and the same 32 other type findings. None is suppressed; additional files join this single bounded path list only after their data-access contracts and focused behavior tests are clean.
